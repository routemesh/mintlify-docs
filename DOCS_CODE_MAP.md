...
| **debugging** | `X-Batch-Id` header name; RPC URL pattern `rpc.routeme.sh/rpc/{chain_id}/{api_key}`; Logs page URL | Atlas or gateway: where response headers are set (BatchID exists in `atlas/api/rpc/handle-rpc-requests.go`, `atlas/common/models/request-context.go`). Front-end logs: `front-end/src/app/(protected)/app/consumer/logs/` |
...
|-------|----------------|------------|
| **api-reference/openapi.json** | Server URLs (routeme.sh, rpc.routeme.sh, lb, lb2); error codes in descriptions; `X-Batch-Id` in responses; path `/rpc/{chain_id}/{api_key}` | `atlas/api/rpc/rpc.go` (route registration); `atlas/common/json-rpc/json-rpc.go` (codes); OpenAPI spec source if generated from code. |
...
|------|----------------|--------|
| **routeme.sh, rpc.routeme.sh, lb.routeme.sh, lb2.routeme.sh** | Docs + openapi.json + intro (redundancy, getting-started, debugging) | Same hostnames in `front-end/src/lib/constants/external-links.ts` and any env/deploy config. |
