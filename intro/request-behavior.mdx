...
This page explains how RouteMesh handles your RPC requests: **allowed domains** for API keys, **cooldowns and rate limiting**, **batch requests**, and **eth_getLogs limits**.

## eth_getLogs limits

RouteMesh enforces a maximum block span of **10,000 blocks** per `eth_getLogs` request when both `fromBlock` and `toBlock` are fixed hex values. Dynamic tags (`latest`, `safe`, `finalized`, `pending`) are not pre-validated against this limit.

When a request exceeds a limit, the router returns a normalized JSON-RPC error with an actionable hint in `error.message`. See [RPC Error Codes](/intro/rpc-error-codes#eth_getlogs-error-messages) for the full code table.

| Situation | Code | What to do |
| :-------- | :--- | :--------- |
| Block span exceeds 10,000 blocks | **-32602** (`block range too large`) | Split the query into smaller `fromBlock`/`toBlock` windows. Use the `; try a block range of at most N blocks` hint when present. |
| Span is within limits but too many logs match | **-32005** (`query exceeds max results`) | Narrow the block range using the hint, or tighten filters (`address`, `topics`). Common for unfiltered or high-volume contracts. |

**Retry pattern:** On `-32005`, halve the block span and retry until the query succeeds, then widen gradually if needed. On `-32602`, stay at or below the hinted maximum span.

**The hint's `N` is your current limit, not always the 10,000 cap.** The `; try a block range of at most N blocks` suffix reports the largest span RouteMesh can currently route for that chain and access tier. This can be smaller than 10,000 and changes over time — different chains and access tiers can have different limits, and the value moves as providers and their limits change. When you see the hint, use its `N` as the maximum for that chain.
